...
if (require("os").platform() === "win32") {
  logFmt =
    'format:{\\"commit\\":\\""%H"\\",' +
    '\\"date\\":\\"%ar\\",\\"message\\":\\"%f\\"},'; //TODO: Fix this... This is only a workaround, we should be using the following line, however it will fail because some commits have double quotes (")
...
router.get("/", function (req, res, next) {
  ...
  return res.render("eng", { hostname: trueHostname, isMobile: isMobile(req) });
});
...
router.get("/gitblog/:lang?/:article", function (req, res, next) {
  ...
  var full_article_path =
    path.join(__dirname, "../public/articles/") + current_file;
    ...
    function (err, msg) {
      ...
      for (var i = 0; i < hashes.length; i++) {
        ...
        var getCommitContent = function (index, next) {
          git.exec("show", [hashes[index] + ":" + current_file], function (
            err,
            msg
          ) {
